...
@RestController
@RequestMapping("auth")
@CrossOrigin(origins = {"http://localhost:4200", "https://portfolio-front-robles.web.app"})
public class AuthController {
    ...
    public ResponseEntity<?> nuevo(@Valid @RequestBody NuevoUsuario nuevoUsuario, BindingResult bindingResult){
        ...
        if(usuarioService.existsByNombreUsuario(nuevoUsuario.getNombreUsuario()))
            return new ResponseEntity(new Mensaje("Ese nombre de usuario ya está registrado, intente con otro."), HttpStatus.BAD_REQUEST);
        
        if(usuarioService.existsByEmail(nuevoUsuario.getEmail()))
            return new ResponseEntity(new Mensaje("Ese email ya esta registrado, intente con otro."), HttpStatus.BAD_REQUEST);
